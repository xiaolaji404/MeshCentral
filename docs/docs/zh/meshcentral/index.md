...
本用户指南包含用户充分利用 MeshCentral（一个免费的开源基于 Web 的远程计算机管理软件）所需的所有基本信息。该指南提供了快速步骤来设置管理组，以在本地网络环境或通过 Internet 远程控制和管理计算机。文档的后半部分将涵盖一些高级主题。读者应该已经对计算机网络、操作系统和网络安全有一些基本的了解。

## 📘 介绍
MeshCentral 是一个免费的开源基于 Web 的远程计算机管理软件。您可以在本地网络或互联网上设置自己的管理服务器，并远程控制和管理运行 Windows* 或 Linux\* 操作系统的计算机。 

![](images/2022-05-18-22-11-41.jpg)

首先，需要一个基础或管理服务器。管理服务器可以是任何具有足够计算、存储和可靠网络组件的计算设备（PC 或 VM），以托管 MeshCentral 环境并在远程管理练习期间提供良好的性能。虽然有许多适用于高级用户的配置，但典型的服务器设置只需几分钟即可完成。

从高层次来看，只有四个 (4) 个主要步骤：**_设置_**、**_安装_**、**_连接_** 和 **_控制_**。

  1. 在 VM 或 PC 上设置 MeshCentral 服务器
  2. 使用有效账户登录 MeshCentral 门户，创建一个管理网格以收集所有端点（要管理的系统）
  3. 生成代理并将其安装在目标或每个端点上，该端点立即尝试连接回 MeshCentral 服务器。
  4. 控制/管理相应管理网格中可用的资产或端点
...
由于 MeshCentral 服务器是用 NodeJS 编写的，因此它可以安装在许多操作系统上，包括 Windows、Linux。有关如何安装服务器的信息，请参阅 MeshCentral 安装指南 <https://www.meshcommander.com/meshcentral2>。
